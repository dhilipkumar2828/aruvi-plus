    ...
    <div class="premium-admin-invoice">
        <div class="order-header-premium-box d-print-none">
            <div style="display: flex; justify-content: space-between; align-items: flex-end;">
                <div>
                    ...
                    <h1 style="font-size: 32px; font-weight: 800; margin: 0; letter-spacing: -0.5px; color: #fff;">Order
                        Invoice</h1>
                ...
                <div style="display: flex; gap: 12px;">
                    <a href="{{ route('admin.orders') }}" class="admin-btn admin-btn-ghost"
                        style="background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2); color: #fff; box-shadow: none;">
                        <i class="fas fa-arrow-left"></i> Back to Orders
                    </a>
                    <a href="{{ route('admin.orders.download', $order) }}" class="admin-btn"
        ...
        <div style="padding: 0 10px;">
            ...
            <div class="glass-card" id="invoice-area">
                ...
                <div class="invoice-box-header"
                    ...
                    <div style="text-align: right;">
                        <h5
                            style="background: #f1f5f9; padding: 4px 12px; border-radius: 6px; font-size: 11px; font-weight: 700; color: #475569; letter-spacing: 1px; text-transform: uppercase; margin-bottom: 10px; display: inline-block;">
                            Tax Invoice</h5>
                        <h3 style="font-size: 22px; font-weight: 700; margin: 0; color: var(--primary);">
                            #{{ $order->order_number }}</h3>
                        <p style="font-size: 13px; color: #94a3b8; margin: 4px 0;">Invoice Date:
                            <strong>{{ $order->created_at->format('d M, Y') }}</strong></p>
                        <p style="font-size: 13px; color: #94a3b8; margin: 4px 0;">Payment Status: <strong
                                style="color: {{ $order->payment_status == 'paid' ? '#059669' : '#dc2626' }};">{{ strtoupper($order->payment_status) }}</strong>
                        </p>
                    </div>
                </div>
                ...
                <div class="address-grid"
                    style="display: grid; grid-template-columns: 2fr 1fr; gap: 40px; margin-bottom: 40px;">
                    <div style="display: grid; grid-template-columns: 1fr; gap: 20px;">
                        <div>
                            <span class="summary-label">Shipping Details</span>
                            <div class="summary-value">
                                <strong
                                    style="display: block; color: #000; margin-bottom: 4px; font-size: 15px;">{{ $order->customer_name }}</strong>
                                {{ $order->phone }}<br>
                    ...
                    <div>
                        ...
                        <div
                            style="background: #f8fafc; padding: 20px; border-radius: 16px; font-size: 14px; border: 1px solid #f1f5f9;">
                            <div style="display: flex; justify-content: space-between; margin-bottom: 12px;">
                                <span style="color: #64748b;">Order Status</span>
                                <span
                                    class="status-badge {{ $order->status == 'completed' || $order->status == 'delivered' ? 'status-success' : 'status-warning' }}"
                                    style="padding: 4px 10px; font-size: 9px; min-width: auto;">{{ strtoupper($order->status) }}</span>
                            </div>
                            <div style="display: flex; justify-content: space-between; margin-bottom: 12px;">
                            ...
                            <div
                                style="display: flex; justify-content: space-between; padding-top: 12px; border-top: 1px dashed #e2e8f0;">
                                <span style="font-weight: 600; color: #1e293b;">Amount Paid</span>
                                <strong
                                    style="color: var(--primary); font-size: 18px;">₹{{ number_format($order->amount, 0) }}</strong>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <tbody>
                                ...
                                @foreach($order->items as $item)
                                    <tr>
                                        ...
                                        <td style="text-align: center;">
                                            ...
                                            @else
                                                <div
                                                    style="width: 50px; height: 50px; background: #f1f5f9; border-radius: 8px; display: flex; align-items: center; justify-content: center; margin: 0 auto;">
                                        ...
                                        <td>
                                            <strong
                                                style="display: block; color: #1e293b; font-size: 14px;">{{ $item->product_name }}</strong>
                                            <span style="font-size: 11px; color: #94a3b8; display: block; margin-top: 2px;">HSN:
                                                {{ $item->hsn ?? $item->product?->hsn ?? '3004' }}</span>
                                        </td>
                                        <td style="text-align: center; color: #1e293b; font-weight: 600;">{{ $item->quantity }}</td>
                                        <td style="text-align: right; color: #64748b;">₹{{ number_format($item->unit_price, 0) }}
                                        </td>
                                        <td style="text-align: right; color: #000; font-weight: 800;">
                            ...
                            @else
                                <tr>
                                    ...
                                    <td>
                                        <strong
                                            style="display: block; color: #1e293b; font-size: 14px;">{{ $order->product_name }}</strong>
                                        <span style="font-size: 11px; color: #94a3b8; display: block; margin-top: 2px;">HSN:
                                            3004</span>
                                    </td>
                                    <td style="text-align: center; color: #1e293b; font-weight: 600;">{{ $order->quantity }}
                                    </td>
                                    <td style="text-align: right; color: #64748b;">
                                        ₹{{ number_format($order->amount / max($order->quantity, 1), 0) }}</td>
                                    <td style="text-align: right; color: #000; font-weight: 800;">
                ...
                <div style="display: flex; justify-content: flex-end;">
                    ...
                    <div style="width: 320px;">
                        <div
                            style="display: flex; justify-content: space-between; padding: 12px 0; font-size: 14px; color: #64748b; border-bottom: 1px solid #f1f5f9;">
                        ...
                        @if($order->discount_amount > 0)
                            <div
                                style="display: flex; justify-content: space-between; padding: 12px 0; font-size: 14px; color: #1a5d1a; border-bottom: 1px solid #f1f5f9;">
                        ...
                        <div
                            style="display: flex; justify-content: space-between; padding: 12px 0; font-size: 15px; color: #1e293b; border-bottom: 1px solid #f1f5f9; background: #fcfcfc;">
                        ...
                        <div
                            style="display: flex; justify-content: space-between; padding: 25px 0; align-items: flex-start;">
                            <div>
                                <span
                                    style="font-size: 18px; font-weight: 800; color: #9d174d; display: block; line-height: 1;">Final
                                    Amount</span>
                                <span style="font-size: 12px; color: #64748b; font-weight: 500;">(Inclusive of GST)</span>
                            </div>
                            <strong
                                style="font-size: 28px; font-weight: 800; color: #9d174d; line-height: 1;">₹{{ number_format($order->amount, 2) }}</strong>
                        </div>
                    </div>
                </div>
                ...
                <div style="margin-top: 80px; display: flex; justify-content: space-between; align-items: flex-start;">
                    <div style="max-width: 450px;">
                        <h6
                            style="font-size: 11px; font-weight: 800; color: #1e293b; text-transform: uppercase; margin-bottom: 8px; letter-spacing: 1px;">
                            Declaration</h6>
                        <p style="font-size: 11px; color: #94a3b8; line-height: 1.8; margin: 0; font-weight: 300;">
                            This is a computer generated invoice and does not require a physical signature. Goods once sold
                            cannot be returned unless found damaged upon arrival.
                        </p>
                    </div>
                    <div style="text-align: left; width: 220px;">
                        ...
                        <p style="font-size: 12px; color: #94a3b8; line-height: 1.4; font-weight: 300; margin: 0;">Auvri
                            Plus</p>
                    </div>
                </div>
            </div>
            ...
            <div class="d-print-none" style="text-align: center; margin-top: 40px; padding-bottom: 40px;">
                <p style="font-size: 13px; color: #94a3b8;">Generated from Admin Terminal |
