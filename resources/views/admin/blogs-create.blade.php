...
<div class="content-card">
    <div class="card-header">
        <div>
            ...
            <p style="margin: 6px 0 0; font-size: 13px; color: var(--text-muted);">Draft and publish a new article to your spiritual blog.</p>
        </div>
    ...
    <form action="{{ route('admin.blogs.store') }}" method="POST" enctype="multipart/form-data" style="padding: 40px;">
        @csrf
        ...
        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 25px;">
            ...
            <div class="admin-form-group">
                <label class="admin-form-label">Upload Image <span class="text-danger">*</span></label>
                <div class="file-upload">
                    <input id="image" name="image" type="file" accept="image/*" required>
                    <label for="image" class="file-button">Choose File</label>
                    <span id="image_name" class="file-name">No file chosen</span>
                    <div id="image_preview_container" style="display: none; margin-left: auto;">
            ...
            <div class="admin-form-group">
                ...
                <input type="date" name="published_at" value="{{ old('published_at', date('Y-m-d')) }}" class="admin-input" required>
            ...
            <div class="admin-form-group" style="grid-column: span 2;">
                ...
                <textarea name="excerpt" rows="3" class="admin-textarea" placeholder="Brief summary of the article for list views..." required>{{ old('excerpt') }}</textarea>
            ...
            <div class="admin-form-group" style="grid-column: span 2;">
                ...
                <textarea name="content" rows="10" class="admin-textarea" placeholder="Write your full article content here..." required>{{ old('content') }}</textarea>
        ...
        <div style="display: flex; justify-content: flex-end; gap: 15px; margin-top: 20px; padding-top: 30px; border-top: 1px solid rgba(0, 66, 0, 0.1);">
            <button type="submit" class="admin-btn admin-btn-primary">
                <i class="fas fa-cloud-upload-alt"></i> Create Blog
            </button>
        </div>
    </form>
</div>
...
<script>
    $(document).ready(function() {
        $('#title').on('input', function() {
            ...
            if (name.length > 2) {
                window.checkUnique('blogs', 'title', name, '#title', {{ isset($blog) ? $blog->id : 'null' }});
            } else {
        ...
        $('#slug').on('input', function() {
            let slug = $(this).val();
            if (slug.length > 1) {
                window.checkUnique('blogs', 'slug', slug, '#slug', {{ isset($blog) ? $blog->id : 'null' }});
            } else {
                $(this).css('border-color', '');
                $(this).siblings('.error-msg').hide();
            }
