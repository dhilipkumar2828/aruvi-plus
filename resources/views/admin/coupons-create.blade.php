...
<div class="content-card">
    <div class="card-header">
        <div>
            ...
            <p style="margin: 6px 0 0; font-size: 13px; color: var(--text-muted);">Define discount rules for customer orders.</p>
        </div>
    ...
    <form action="{{ route('admin.coupons.store') }}" method="POST" style="padding: 40px;">
        @csrf

        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 25px;">
            <div class="admin-form-group" style="grid-column: span 2;">
                <label class="admin-form-label">Applicable Products (Optional)</label>
                <p style="font-size: 12px; color: #888; margin-bottom: 10px;">If no products are selected, the coupon will apply to the entire cart subtotal. If products are selected, the discount ONLY applies to those products.</p>
            ...
            <div class="admin-form-group">
                ...
                <input type="text" name="description" value="{{ old('description') }}" class="admin-input" placeholder="e.g. 10% off for new devotees">
            </div>

            <div class="admin-form-group">
                <label class="admin-form-label">Discount Type <span class="text-danger">*</span></label>
            ...
            <div class="admin-form-group">
                <label class="admin-form-label">Discount Value <span class="text-danger">*</span></label>
                <input type="number" name="value" value="{{ old('value') }}" step="0.01" min="0" required class="admin-input" placeholder="e.g. 10 or 250">
            </div>

            <div class="admin-form-group">
                <label class="admin-form-label">Minimum Order Amount</label>
                <input type="number" name="minimum_amount" value="{{ old('minimum_amount') }}" step="0.01" min="0" class="admin-input" placeholder="Optional">
            </div>

            <div class="admin-form-group">
                <label class="admin-form-label">Maximum Discount Cap</label>
                <input type="number" name="maximum_discount" value="{{ old('maximum_discount') }}" step="0.01" min="0" class="admin-input" placeholder="Optional">
            </div>

            <div class="admin-form-group">
                <label class="admin-form-label">Starts On</label>
                <input type="date" name="starts_at" value="{{ old('starts_at') }}" class="admin-input">
            </div>

            <div class="admin-form-group">
                <label class="admin-form-label">Ends On</label>
                <input type="date" name="ends_at" value="{{ old('ends_at') }}" class="admin-input">
            </div>

            <div class="admin-form-group">
                <label class="admin-form-label">Usage Limit</label>
                <input type="number" name="usage_limit" value="{{ old('usage_limit') }}" min="1" class="admin-input" placeholder="Optional">
            </div>
        ...
        <div style="display: flex; justify-content: flex-end; gap: 15px; margin-top: 20px; padding-top: 30px; border-top: 1px solid rgba(194, 24, 91, 0.1);">
            ...
            <button type="submit" class="admin-btn admin-btn-primary">
                <i class="fas fa-tags"></i> Save Coupon
            </button>
        </div>
    </form>
</div>
